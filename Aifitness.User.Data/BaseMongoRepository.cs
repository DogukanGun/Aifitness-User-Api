...
{
    public class BaseMongoRepository<TDocument> : IMongoRepository<TDocument> where TDocument : IDocument
    {
        ...
        {
            #if TRACE
            var traceSource = new TraceSource("mongo", SourceLevels.Verbose);
        ...
        public BaseMongoRepository(IOptions<MongoDbOptions> options)
        {
            var database = BuildMongoClient(options.Value.ConnectionString).GetDatabase(options.Value.DatabaseName);
            var name = GetCollectionName(typeof(TDocument));
            _collection = database.GetCollection<TDocument>(name);
        }
